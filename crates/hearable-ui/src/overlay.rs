//! egui/eframe always-on-top caption window (the `ui` feature). macOS + X11; the Wayland
//! layer-shell path is Phase 3. Rendering only — display state lives in [`crate::CaptionView`].
...
/// The overlay application: drains caption events into the view-model and renders them in a
/// scrollable history, offering an inline "name" field for each as-yet-unnamed speaker cluster.
...
impl CaptionOverlay {
    pub fn new(rx: Receiver<CaptionEvent>, commands: Sender<UiCommand>, history: usize) -> Self {
        Self {
            view: CaptionView::new(history),
            rx,
            commands,
...
impl eframe::App for CaptionOverlay {
    fn clear_color(&self, _visuals: &egui::Visuals) -> [f32; 4] {
        [0.0, 0.0, 0.0, 0.0] // transparent so the dark caption panel reads as a translucent card
    }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().frame(panel).show(ctx, |ui| {
            // Scrollable history: auto-follows the newest line, but the user can scroll up.
...
/// Launch the overlay window. Consumes caption events from `rx` and forwards naming actions on
/// `commands`. `history` is how many past lines to keep scrollable. Blocks until the window closes.
pub fn run_overlay(
    rx: Receiver<CaptionEvent>,
    commands: Sender<UiCommand>,
    history: usize,
    ...
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("hearable")
            .with_inner_size([860.0, 460.0])
            .with_min_inner_size([360.0, 160.0])
            // Decorations on so the window is movable, resizable, and closable; still
            // always-on-top with a translucent caption panel.
            .with_resizable(true)
    ...
    eframe::run_native(
        "hearable",
        options,
        Box::new(move |_cc| Ok(Box::new(CaptionOverlay::new(rx, commands, history)))),
    )
}
